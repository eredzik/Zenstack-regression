...
async function main() {
  ...
  // Extra comments on same posts (ordering / cardinality stress)
  ...
  // u2: 40 posts with overlapping sequence/title patterns for OR/AND tests
  ...
  // u4: only high-sequence posts → satisfies posts.none({ sequence: { lt: 5 } })
  ...
  // u5: mix of low and high (does NOT satisfy none lt 5)
  await prisma.post.createMany({
    data: [
      {
        id: "m1",
  ...
  // u6, u7: a few posts for groupBy / OR on authorId diversity
  await prisma.post.createMany({
    data: [
      {
        id: "a1",
